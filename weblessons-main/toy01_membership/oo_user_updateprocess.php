<!-- 
  ...
  기능: 
  oo_user_update.php 사용자정보 수정화면에서 입력된 값을 받아, 
  users 테이블에 수정된 사용자 데이터를 업데이트.
-->
...
<?php
  ...
  // 데이터베이스 작업 전, 회원정보수정 화면으로 부터 값을 전달 받고
  $id = $_POST['id'];
  $username = $_POST['username']; //수정은 하지 않지만 값은 받음.
...
//   echo "<script>alert($id)</script>";
  ...
  if($conn->connect_error) {
    ...
    $stmt->bind_param("ssi", $cellphone, $email, $id);
    $stmt->execute();
